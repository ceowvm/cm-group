...
function diagnose(){let a=state.answers,p='education';if(a.interest==='automation'||a.goal==='automate'||['emotions','discipline'].includes(a.main_barrier))p='cmlab';else if(a.interest==='signals'||['unstable','losses'].includes(a.experience))p='signals';let profile={title:'Начинающий инвестор',subtitle:'Вам важно выстроить базу и понять риски до активных действий.'};if(['unstable','losses'].includes(a.experience))profile={title:'Трейдер с опытом, но без системного подхода',subtitle:'Главный резерв роста — дисциплина, повторяемость решений и контроль рисков.'};else if(a.interest==='automation')profile={title:'Инвестор, готовый к системной автоматизации',subtitle:'Вам подходит формат, который снижает влияние эмоций и ручных ошибок.'};else if(a.interest==='signals')profile={title:'Практичный трейдер, ориентированный на готовые решения',subtitle:'Вам важны скорость, структура и понятные рекомендации.'};state.recommendedProduct=p;state.profile=profile;state.quizCompleted=true;save()}
...
function gifts(){return `<section class="screen stack"><div class="card"><span class="profile-badge">Стартовая подборка</span><h2 class="mt-16">${state.name?esc(state.name)+', ':''}ваши материалы готовы</h2><p class="muted">Видео уже доступны внутри Mini App — их можно посмотреть, не возвращаясь в бот.</p></div><div class="card"><h3>CM Signals — готовое решение</h3><p class="muted">Как работают торговые сигналы и сопровождение эксперта.</p><div class="video-frame mt-16"><video controls playsinline preload="metadata" src="https://files.salebot.pro/uploads/file_item/48111350/file/657680/CM_Signals_-_готовое_решение_для_экономии_вашего_времени_и_сил-original.mp4"></video></div></div><div class="card"><h3>CM Lab — автоматизированная торговая студия</h3><p class="muted">Как устроена автоматизация торговли и выбор конфигурации.</p><div class="video-frame mt-16"><video controls playsinline preload="metadata" src="https://files.salebot.pro/uploads/file_item/48111473/file/657680/CM_LAB_-_автоматизированная_торговая_студия__2_.mp4"></video></div></div><div class="card"><h3>Дополнительные материалы</h3><div class="unlock"><div><strong>Книга</strong><div class="muted">Дневник успешного трейдера</div></div><strong class="ok">Открыто</strong></div><div class="unlock"><div><strong>Тестовый доступ</strong><div class="muted">К продуктам CM Group</div></div><strong class="lock">Закрыто</strong></div><div class="unlock"><div><strong>Финансовый калькулятор</strong><div class="muted">Два сценария роста капитала</div></div><strong class="lock">Закрыто</strong></div><button class="btn btn-primary mt-16" data-nav="phone">Открыть полный доступ</button><button class="btn btn-ghost mt-12" data-action="skip-phone">Продолжить без консультации</button></div></section>`}
...
function dashboard(){let c=[state.quizCompleted,state.calculatorCompleted,state.phoneSubmitted].filter(Boolean).length;return `<section class="screen stack"><div class="hero"><div class="eyebrow">Личный кабинет</div><h1>${state.name?'Здравствуйте, '+esc(state.name):'Ваш персональный кабинет'}</h1><p class="lead">Диагностика сохранена. Продолжайте знакомство с возможностями CM Group.</p><div class="kpi-row"><div class="kpi"><strong>${state.profile?.title?'Готов':'—'}</strong><span>профиль</span></div><div class="kpi"><strong>${c}/3</strong><span>этапа</span></div><div class="kpi"><strong>${state.phoneSubmitted?'100%':'67%'}</strong><span>доступ</span></div></div></div><div class="grid two"><button class="card" data-nav="result"><strong>Мой результат</strong></button><button class="card" data-nav="calculator"><strong>Калькулятор</strong></button><button class="card" data-nav="materials"><strong>Материалы</strong></button><button class="card" data-nav="products"><strong>Продукты</strong></button></div><button class="btn btn-primary" data-action="return-bot">Вернуться в Telegram-бот</button><button class="btn btn-ghost" data-action="reset">Сбросить демо</button></section>`}
function calculator(){let c=+state.calc.capital,m=+state.calc.months,s=c*Math.pow(1.17,m),l=c*Math.pow(1.12,m),max=Math.max(s,l);let poly=r=>Array.from({length:m+1},(_,i)=>{let v=c*Math.pow(1+r,i),x=12+i/m*296,y=160-v/max*130;return `${x},${y}`}).join(' ');return `<section class="screen stack"><div class="card"><span class="profile-badge">Финансовый калькулятор</span><h2 class="mt-16">Рассчитайте два сценария</h2><div class="field"><label>Стартовый капитал</label><input id="capital" type="number" min="3000" max="20000" step="500" value="${c}"></div><div class="field mt-16"><label>Срок, месяцев</label><select id="months">${[3,4,5,6,9,12].map(x=>`<option ${x===m?'selected':''}>${x}</option>`).join('')}</select></div><button class="btn btn-primary mt-16" data-action="calculate">Рассчитать</button></div><div class="card"><div class="chart"><svg viewBox="0 0 320 180" preserveAspectRatio="none"><polyline class="chart-line-a" points="${poly(.17)}"/><polyline class="chart-line-b" points="${poly(.12)}"/></svg></div><div class="grid two"><div class="stat"><span class="muted">CM Signals</span><strong>${money(s)}</strong></div><div class="stat"><span class="muted">CM Lab</span><strong>${money(l)}</strong></div></div><p class="disclaimer mt-16">Расчёт носит информационный характер и не является гарантией доходности или индивидуальной инвестиционной рекомендацией.</p></div></section>`}
function materials(){let lock=!state.phoneSubmitted;return `<section class="screen stack"><div class="card"><span class="profile-badge">Материалы</span><h2 class="mt-16">Ваша обучающая подборка</h2><p class="muted">Смотрите видео прямо внутри Mini App и переходите к подробному описанию продукта.</p></div><div class="card"><h3>CM Signals</h3><p class="muted">Готовые торговые рекомендации, обучение и сопровождение эксперта.</p><div class="video-frame mt-16"><video controls playsinline preload="metadata" src="https://files.salebot.pro/uploads/file_item/48111350/file/657680/CM_Signals_-_готовое_решение_для_экономии_вашего_времени_и_сил-original.mp4"></video></div><button class="btn btn-secondary mt-16" data-action="open-product" data-product="signals">Подробнее о CM Signals</button></div><div class="card"><h3>CM Lab</h3><p class="muted">Автоматические торговые студии, работающие по системному алгоритму.</p><div class="video-frame mt-16"><video controls playsinline preload="metadata" src="https://files.salebot.pro/uploads/file_item/48111473/file/657680/CM_LAB_-_автоматизированная_торговая_студия__2_.mp4"></video></div><button class="btn btn-secondary mt-16" data-action="open-product" data-product="cmlab">Подробнее о CM Lab</button></div><div class="card"><h3>Книга</h3><p class="muted">Дневник успешного трейдера</p><button class="btn btn-secondary" data-action="demo-material">Открыть материал</button></div><div class="card"><h3>Консультации</h3><p class="muted">Персональная работа с экспертом CM Group.</p><button class="btn ${lock?'btn-ghost':'btn-secondary'}" data-action="${lock?'need-phone':'product-consultation'}" data-product="${state.recommendedProduct||'signals'}">${lock?'Открыть доступ':'Запросить консультацию'}</button></div></section>`}
function products(){
let a=[
